"""Script: party vs cada boss, verificando transição de fase."""
...
def main() -> None:
    ...
    factory = BossFactory(weapons, profiles, skills)
    bosses = load_all_bosses()

    print("=" * 60)
    print("  BOSS FIGHT TEST")
    print("=" * 60)

    for boss_id, template in bosses.items():
        party = _make_party()
        ...
        initial_hp = boss.max_hp
        combat_result = engine.run_combat()
        final_hp = boss.current_hp
        final_phase = result.handler.current_phase

        status = "VITORIA" if combat_result == CombatResult.PARTY_VICTORY else "DERROTA"
        if combat_result == CombatResult.DRAW:
            status = "EMPATE"

        print(f"\n  Boss: {boss.name}")
        print(f"    Resultado: {status}")
        print(f"    Rounds: {engine.round_number}")
        print(f"    Boss HP: {initial_hp} -> {final_hp}")
        print(f"    Fase final: {final_phase}")
        print(f"    Eventos: {len(engine.events)}")
