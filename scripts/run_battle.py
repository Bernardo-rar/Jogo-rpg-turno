"""Roda Mock Battle v2 e printa o combat log no terminal."""
...
def main():
    ...
    print(f"\n{'='*50}")
    print(f"  MOCK BATTLE V2 ({engine.round_number} rounds)")
    print(f"{'='*50}\n")
    print(text)
    print(f"\n{'='*50}")
    ...
    for c in party + enemies:
        if c.current_hp <= 0:
            ...
            print(f"  {c.name}: HP={c.current_hp}/{c.max_hp}  Mana={c.current_mana}/{c.max_mana}")
